...
Fetch live queue status and/or data freshness for up to 20 checkpoints in a single request. Designed for dashboard builders who poll many PPIDs simultaneously — reduces 20+ individual calls to one. Quota counts as N PPIDs × number of sub-products requested (same total as individual calls, but one round-trip).
